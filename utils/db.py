...
def sb_execute(query, reintentos=3, espera=1):
    """Ejecuta query de Supabase con reintentos automáticos ante errores de red."""
    ultimo_error = None
...
def init_db():
    try:
        sb = get_supabase()
        sb_execute(sb.table("colaboradores").select("id").limit(1))
    except Exception as e:
...
def get_colaborador(numero_emp: str):
    sb = get_supabase()
    res = sb_execute(sb.table("colaboradores").select("*").eq("numero_emp", numero_emp))
    return res.data[0] if res.data else None

def crear_colaborador(nombre: str, numero_emp: str):
    sb = get_supabase()
    try:
        res = sb_execute(sb.table("colaboradores").insert({
...
def get_all_colaboradores():
    sb = get_supabase()
    res = sb_execute(sb.table("colaboradores").select("*").order("nombre"))
    return res.data or []

# ── Partidos ──────────────────────────────────────────────────────────────────
def get_partidos():
    sb = get_supabase()
    res = sb_execute(sb.table("partidos").select("*").order("fecha").order("id"))
    return res.data or []

def get_partido(partido_id: int):
    sb = get_supabase()
    res = sb_execute(sb.table("partidos").select("*").eq("id", partido_id))
    return res.data[0] if res.data else None
...
def upsert_partido(partido_id, goles_local, goles_visita, cerrado=1, equipo_pasa=None):
    ...
    sb_execute(sb.table("partidos").update(data).eq("id", partido_id))
    _recalcular_puntos(partido_id)

def agregar_partido(fase, local, visita, fecha, hora=None, sede=None):
    sb = get_supabase()
    sb_execute(sb.table("partidos").insert({
...
def eliminar_partido(partido_id: int):
    sb = get_supabase()
    sb_execute(sb.table("pronosticos").delete().eq("partido_id", partido_id))
    sb_execute(sb.table("partidos").delete().eq("id", partido_id))
...
def upsert_pronostico(colaborador_id, partido_id, goles_local, goles_visita):
    ...
    if existing:
        sb_execute(sb.table("pronosticos").update({
            "goles_local": goles_local, "goles_visita": goles_visita
        }).eq("id", existing["id"]))
    else:
        sb_execute(sb.table("pronosticos").insert({
...
def get_pronosticos_partido(partido_id):
    sb = get_supabase()
    res = sb_execute(sb.table("pronosticos").select("*").eq("partido_id", partido_id))
    rows = []
    for r in (res.data or []):
        try:
            col = sb_execute(sb.table("colaboradores").select("nombre").eq("id", r["colaborador_id"]))
...
def _recalcular_puntos(partido_id):
    ...
    for p in (res.data or []):
        ...
        sb_execute(sb.table("pronosticos").update({"puntos": pts}).eq("id", p["id"]))

def get_tabla_general():
    sb = get_supabase()
    cols = sb_execute(sb.table("colaboradores").select("*").eq("es_admin", 0))
    tabla = []
    for c in (cols.data or []):
        res = sb_execute(sb.table("pronosticos").select("puntos").eq("colaborador_id", c["id"]))
        prons = res.data or []
...
def numero_en_lista_blanca(numero_emp: str) -> bool:
    sb = get_supabase()
    res = sb_execute(sb.table("lista_blanca").select("id").eq("numero_emp", numero_emp))
...
def cargar_lista_blanca(numeros: list):
    ...
    for item in numeros:
        ...
        if not numero or numero == "None":
            continue
        try:
            sb_execute(sb.table("lista_blanca").upsert({
                "numero_emp": numero, "nombre_ref": nombre
            }, on_conflict="numero_emp"))
            insertados += 1
        except Exception:
            pass
    return insertados
...
def get_lista_blanca():
    sb = get_supabase()
    res = sb_execute(sb.table("lista_blanca").select("*").order("numero_emp"))
    return res.data or []

def eliminar_de_lista_blanca(numero_emp: str):
    sb = get_supabase()
    sb_execute(sb.table("lista_blanca").delete().eq("numero_emp", numero_emp))

def lista_blanca_activa() -> bool:
    sb = get_supabase()
    res = sb_execute(sb.table("lista_blanca").select("id").neq("numero_emp", "0000"))
    return len(res.data) > 0
...
def agregar_a_lista_blanca(numero_emp: str, nombre_ref: str = "") -> bool:
    ...
    if res.data:
        return False
    sb_execute(sb.table("lista_blanca").insert({
...
def eliminar_colaborador(colaborador_id: int):
    sb = get_supabase()
    sb_execute(sb.table("pronosticos").delete().eq("colaborador_id", colaborador_id))
    sb_execute(sb.table("colaboradores").delete().eq("id", colaborador_id))
